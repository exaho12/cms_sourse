            ...
            if (D('Paymentlogs')->save($data)) { //总之 先更新 然后再处理逻辑  这里保障并发是安全的
                ...
                if ($logs['type'] == 'gold') {
                   ...
                   $member = D('Users') -> find($logs['user_id']);
					...
					$_data_balance = array('url' => 'http://' . $_SERVER['HTTP_HOST'] . '/mcenter/index/index', 'topcolor' => '#F55555', 'first' => '您的账户余额发生变动，信息如下：', 'remark' => '如对上述余额变动有异议，请联系客服人员协助处理。' . $this -> CONFIG['site']['tel'], 'accountType' => '会员账户', 'operateType' => '费用支出', 'operateInfo' => '购物消费', 'limit' => '-' . $limit . '元', 'balance' => $balance . '元');
    ...
    public function mallSold($order_ids) {
        ...
        if (is_array($order_ids)) {
            ...
            foreach ($ordergoods as $k => $v) {
				...
				D('Goodsattr') -> updCount($v['goods_attr_id'],'buynum',$v['num']);
				D('Goodsattr') -> updCount($v['goods_attr_id'],'number',-$v['num']);
				//多属性结束
