...
class NewsAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Communitynews');
            if ($obj->add($data)) {
                $this->niuMsg('物业通知发布成功', U('news/index'));
            }
            $this->niuMsg('操作失败！');
        } else {
            $this->display();
    ...
    public function checkCreate() {
        ...
        if ($words = D('Sensitive')->checkWords($data['title'])) {
            $this->niuMsg('物业通知标题含有敏感词：' . $words);
        }
        $data['intro'] = htmlspecialchars($data['intro']);
        if (empty($data['intro'])) {
            $this->niuMsg('物业通知简介不能为空');
        }
        if ($words = D('Sensitive')->checkWords($data['intro'])) {
            $this->niuMsg('物业通知简介含有敏感词：' . $words);
        }
        $data['details'] = htmlspecialchars($data['details']);
        if (empty($data['details'])) {
            $this->niuMsg('物业通知内容不能为空');
        }
        if ($words = D('Sensitive')->checkWords($data['details'])) {
            $this->niuMsg('物业通知内容含有敏感词：' . $words);
        }
    ...
    public function editCheck($news_id) {
        ...
        if (empty($data['intro'])) {
            ...
            $this->niuMsg('简介含有敏感词：' . $words);
        }$data['details'] = htmlspecialchars($data['details']);
        if (empty($data['details'])) {
            $this->niuMsg('详情不能为空');
        }if ($words = D('Sensitive')->checkWords($data['details'])) {
            $this->niuMsg('详情含有敏感词：' . $words);
        }
		$data['audit'] = 0;//编辑后重审
        return $data;
    }
