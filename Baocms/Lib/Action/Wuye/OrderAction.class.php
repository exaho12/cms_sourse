...
class OrderAction extends CommonAction {
        ...
        if ($this->isPost()) {
            ...
            if (!$res = $obj->where(array('user_id' => $user_id, 'order_date' => $data['order_date']))->find()) {
                if ($order_id = $obj->add($data)) {
                    ...
                    $this->niuMsg('添加成功', U('order/index', array('user_id' => $user_id)));
                } else {
                    $this->niuMsg('添加失败');
                }
            }
            $this->niuMsg('该账单已存在',U('order/index', array('user_id' => $user_id)));
        } else {
    ...
    public function edit($order_id) {
        ...
        if ($this->isPost()) {
            ...
            $this->niuMsg('修改成功', U('order/index', array('user_id' => $detail['user_id'])));
        } else {
