...
class WorkerAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($result) {
				...
				if($msg_id){
					$this->niuMsg('添加成功，已经为该用户发送系统信息，等待他确认！', U('worker/index'));
				}else{
					$this->error('操作失败！');
				}
    ...
    public function edit($worker_id = 0) {
        ...
        if ($this->isPost()) {
            ...
            if (false !== $obj->save($data)) {
                $this->niuMsg('操作成功', U('worker/index', array('worker_id' => $worker_id)));
            }
            $this->niuMsg('操作失败');
        } else {
            $this->assign('detail', $detail);
    ...
    private function editCheck() {
        ...
        if (empty($data['work'])) {
            $this->niuMsg('员工职务不能为空');
        }
        if (empty($data['addr'])) {
            $this->niuMsg('联系地址不能为空');
        }
