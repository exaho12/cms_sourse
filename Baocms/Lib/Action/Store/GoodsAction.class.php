   ...
   //检测微店
	...
	public function weidian() {
        ...
        if ($this->isPost()) {
            ...
            if (empty($data['pic'])) {
                $this->niuMsg('店铺图标没有上传');
            }
           
            if (empty($data['lng']) || empty($data['lat'])) {
                $this->niuMsg('店铺坐标没有选择');
            }
            ...
            if (!$weidian) { //如果没有结果则添加
                ...
                if (!$add) {
                    ...
                    $this->niuMsg('设置成功', U('goods/weidian'));
                }
            } else {   //否则修改
                ...
                if (!$up) {
                    ...
                    $this->niuMsg('修改成功', U('goods/weidian'));
                }
            }
        } else {
            //冗余信息
    ...
    public function create() {
        ...
        if ($this->isPost()) {
            ...
            if ($goods_id = $obj->add($data)) {
                ...
                $this->niuMsg('添加成功,请等待审核', U('store/mart/index'));
            }
            $this->niuMsg('操作失败！');
        } else {
    ...
    private function createCheck() {
        ...
        if ($data['commission'] < 0 || $data['commission']>=$data['settlement_price']) {
            $this->niuMsg('佣金不能为负数，并且不能大于结算价格');
        } 
        ...
        if (empty($data['details'])) {
            $this->niuMsg('商品详情不能为空');
        }
        if ($words = D('Sensitive')->checkWords($data['details'])) {
            $this->niuMsg('商品详情含有敏感词：' . $words);
        } $data['end_date'] = htmlspecialchars($data['end_date']);
        if (empty($data['end_date'])) {
            $this->niuMsg('过期时间不能为空');
        }
        if (!isDate($data['end_date'])) {
            $this->niuMsg('过期时间格式不正确');
        }
		//服务标志
        ...
        if ($goods_id = (int) $goods_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {				
                    $this->niuMsg('编辑成功，请联系管理员审核', U('store/mart/index'));
                }
                $this->niuMsg('操作失败');
            } else {
