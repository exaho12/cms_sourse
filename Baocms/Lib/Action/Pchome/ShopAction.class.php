...
class ShopAction extends CommonAction {
    ...
    public function dianping() {
        ...
        if ($dianping_id = D('Shopdianping')->add($data)) {
            ...
            $photos = $this->_post('photo', false);//这有问题的哦，图片没有写入到数据库！
...
public function yuyue() {
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
				...
				$ytime = strtotime($detail['yuyue_date']);

				$ntime = time();

				if($ytime>$ntime && !empty($detail['mobile'])){

					D('Sms')->sendSms('sms_shop_yue', $detail['mobile'], array(
