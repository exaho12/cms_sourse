...
class CommunityAction extends CommonAction {
        ...
        if ($this->isPost()) {
            ...
            if (!$res = $obj->where(array('community_id' => $community_id, 'user_id' => $this->uid))->find()) {
                ...
                if ($obj->add($data)) {
                    ...
                    $this->niuMsg('申请成功，请等待物业审核',U('community/detail',array('community_id'=>$community_id)));
    ...
    public function detail($community_id) {
        ...
        $this->assign('products_pay',$products_pay = D('Communityorderproducts')->where(array('order_id' => array('IN', $order_ids),'is_pay' => 0))->count());
