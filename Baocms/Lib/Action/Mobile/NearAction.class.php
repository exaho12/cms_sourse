...
class NearAction extends CommonAction {
    ...
    public function loaddata() {
		...
		if(session('city_code')==''){
			$place = D('Cloud')->GetAddress($lat,$lng);//通过云端获取的，需要修改为本地GetAddress
		...
		$arr = D('Cloud')->NearData($lat,$lng,$key,$p);//通过云端获取的，需要修改为本地NearData
		
		$num = intval($arr->total);
		$result = $arr->results;
		
		//自动采集带黄页数据库，现在要求增加一个采集到商家数据库,大王，我现在需要能采集数据，然后把采集的数据导入到商家数据表中任由客户认领！
		if($num > 0 ){
			foreach ($result as $value){
				...
				if($this->_CONFIG['site']['citycode'] == $city_code ){
					...
					if(!empty($pois['pois_id'])){
						...
						if( $time_plus >1314871 && $pois['is_lock']==0){//采集时间
							D('Near')->save($data);
						}
					}else{
						D('Near')->add($data);//插入到数据库
					}
				}
				$poi[] = $data; 
			}
