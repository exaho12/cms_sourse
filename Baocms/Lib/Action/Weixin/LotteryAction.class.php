				...
				if ($record['usenums'] >= $Lottery['max_num'] ) {
					...
					foreach($record as $k => $v){
						...
						if ($thisDayNums>=$Lottery['daynums']){
							...
							//
                            $time = time();
							$data_sn = array('uid'=>$member['uid'],'openid'=>$openid,'shop_id'=>$shop_id,'lottery_id'=>$Lottery['id'],'nickname'=>$wx_info['nickname'],'islottery'=>$isLottery,'sn'=>$data['sncode'],'prize'=>intval($data['prizetype']),'dateline'=>$time);
