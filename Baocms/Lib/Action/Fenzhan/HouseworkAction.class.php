...
class  HouseworkAction extends CommonAction{
	 private $create_fields = array('id', 'cate_id','shop_id','price', 'unit','gongju', 'photo','thumb', 'name', 'tel', 'biz_time', 'title', 'contents', 'yuyue_num', 'views', 'areas');
    private $edit_fields = array('id', 'cate_id','shop_id','price', 'unit','gongju', 'photo', 'thumb','name', 'tel', 'biz_time', 'title', 'contents', 'yuyue_num', 'views', 'areas');
    ...
    public function index(){
        ...
        if ($keyword) {
            $maps['name|tel|contents'] = array('LIKE', '%' . $keyword . '%');
        ...
        $count = $Housework->where($maps)->count(); // 查询满足要求的总记录数 
        $Page = new Page($count, 15); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
        $list = $Housework->where($maps)->order(array('housework_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function edit($housework_id){
        if ($housework_id = (int) $housework_id) {
			...
			//查询上级ID编辑处代码开始
			$shop_ids = D('Housework')->find($housework_id);
			$shop_id = $shop_ids['shop_id'];
			
			if (empty($shop_id)) {
			   $this->error('必须关联商家，联系总管', U('housework/index'));
			}
			
			$city_ids = D('Shop')->find($shop_id);
			$citys = $city_ids['city_id'];
			if ($citys != $this->city_id) {
			   $this->error('非法操作', U('housework/index'));
			}
